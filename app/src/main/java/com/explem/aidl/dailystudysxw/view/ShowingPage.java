package com.explem.aidl.dailystudysxw.view;
...
import com.explem.aidl.dailystudysxw.utils.CommonUtils;
...
public abstract class ShowingPage extends FrameLayout implements View.OnClickListener {
    ...
    public ShowingPage(Context context) {
        ...
        if(showingpage_unload==null){
            ...
            ImageView unload_img= (ImageView) showingpage_unload.findViewById(R.id.unload_img);
            unload_img.setImageResource(R.drawable.lottery_animlist);
            AnimationDrawable animationDrawable = (AnimationDrawable) unload_img.getDrawable();
    ...
    public void onClick(View v) {
        //重新加载界面的方法
        if(reShowingListener!=null){
            if(httpBoolean.checkNetworkState(MyApplication.getConText())){
                Toast.makeText(MyApplication.getConText(), "网络连接错误！", Toast.LENGTH_SHORT).show();
            }else {
                reShowingListener.MyReshowingListener(v);
            }
        }else{
            Toast.makeText(MyApplication.getConText(), "网络连接错误！", Toast.LENGTH_SHORT).show();
        }
